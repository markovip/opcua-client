...
var doDebug = false;
var debugLog = function(){ if(doDebug) { console.log.apply(console,arguments); }};
...
function validateDataType(__address_space, dataTypeNodeId, variantDataType, nodeId) {
    ...
    var destUADataType = __address_space.findNode(dataTypeNodeId);
...
function isSameVariant(va,vb) {
    assert( va instanceof Variant);
    assert( vb instanceof Variant);
    if (va.dataType !== vb.dataType) { return false; }
    if (va.arrayType !== vb.arrayType) { return false; }

    return (_.isEqual(va.value,vb.value));
}
/**
...
UAVariable.prototype.isReadable = function () {
...
UAVariable.prototype.readValue = function (indexRange, dataEncoding) {
    ...
    if (dataValue.statusCode === StatusCodes.BadNodeIdUnknown) {
        debugLog(" Warning:  ".red +  self.browseName.cyan+ ", "+ self.nodeId.toString(), " exists but dataValue has not been defined");
    }
    return dataValue;
};
...
UAVariable.prototype.setValueFromSource = function (variant, statusCode) {
    ...
    if (variant.hasOwnProperty("value")) {
        if (!variant.dataType) {
            throw new Error("Variant must provide a valid dataType");
        }
    }
...
UAVariable.prototype.writeValue = function (dataValue, indexRange, callback) {
    ...
    _apply_default_timestamps(dataValue);
    ...
    self._timestamped_set_func(dataValue, indexRange, function (err, statusCode, correctedDataValue) {
        ...
        if (!err) {
            ...
            assert(correctedDataValue.serverTimestamp);
...
UAVariable.prototype.writeAttribute = function (writeValue, callback) {
    ...
    // use these values.
    _apply_default_timestamps(writeValue.value);
...
function _Variable_bind_with_simple_get(options) {
    ...
    function timestamped_get_func_from__Variable_bind_with_simple_get() {
        ...
        if (is_StatusCode(value)) {
            ...
            if (!self._dataValue || !isSameVariant(self._dataValue.value,value) || self._dataValue.statusCode !== StatusCodes.Good) {
...
UAVariable.prototype.clone = function (options, extraInfo) {
    ...
    var newVariable = self._clone(UAVariable, options, extraInfo);
...
function _bindComponent(variable,addressSpace,prop, name,dataTypeId, valueRank) {
    ...
    prop.setValueFromSource = function(variant,statusCode) {
        var dv = variable.readValue();
